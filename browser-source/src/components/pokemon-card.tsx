...
function usePokeTypes(dexNum: number): string[] {
  ...
  useEffect(() => {
    ...
    fetch(`https://pokeapi.co/api/v2/pokemon/${dexNum}`)
      ...
      .then((data) => {
        const t: string[] = (
          data.types as { slot: number; type: { name: string } }[]
        )
...
function PokemonRow({ mon }: { mon: PartyPokemon }) {
  ...
  return (
    <div
      className={`flex flex-col gap-1.5 min-w-0 px-2 rounded-2xl bg-muted ${fainted ? "opacity-40" : ""}`}
    >
      <div className="flex flex-col items-start justify-around min-w-0">
        <div
          className="h-32 mt-2 self-center p-4 border rounded-full mb-1"
          style={{
            backgroundColor:
              (TYPE_COLORS[types[0]]
                ?.split(" ")[0]
        ...
        >
          <img
            src={`https://raw.githubusercontent.com/HybridShivam/Pokemon/refs/heads/master/assets/thumbnails/${species}.png`}
            alt={mon.nickname}
            className="h-full"
          />
        </div>
        <div className="font-semibold text-sm truncate flex justify-start items-center gap-1 w-full">
          {mon.nickname || `#${mon.species}`}{" "}
          {types.length > 0 &&
            types.map((t) => (
              <span
                key={t}
                className={`text-[9px] px-1.5 py-px rounded font-medium uppercase tracking-wide ${TYPE_COLORS[t] ?? "bg-zinc-600 text-white"}`}
              >
                {t}
              </span>
            ))}
        </div>
        <div className="text-foreground text-xs tabular-nums shrink-0">
          Lv. {mon.level}{" "}
          {status && (
            <span className="text-[10px] px-1 py-px rounded bg-yellow-800/60 text-yellow-300 shrink-0">
              {status}
            </span>
          )}
        </div>
